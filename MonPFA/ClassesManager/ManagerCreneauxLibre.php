...
class ManagerCreneauxLibre{
    ...
    public function fct_findCreneaux(){ /* type tableau des creneaux */
        $creneaux = array();
        $result = $this->db->query("select cl.* from creneaux_libre cl,etudiant e
    ...
    public function fct_insert(ClasseCreneauxLibre $creneaux){ /* type boolean */
        $stmt = $this->db->prepare("insert into creneaux_libre(semaine, jour, heure,cin) values(?,?,?,?);");
    ...
    public function fct_delete($id_creneau){ /* type boolean */
        $stmt = $this->db->prepare("delete from creneaux_libre where id_creneau=?;");
